...
def dice_icons(values):
    """Render a list of die values as Unicode die-face icons."""
    if not values:
        return "—"
    return " ".join(DIE_FACES.get(v, str(v)) for v in values)


def dice_count_icons(count, max_show=6):
    """Render a player's remaining dice count as a row of small die icons."""
    if count <= 0:
        return "❌ eliminated"
    return "🎲" * min(count, max_show)


def probability_badge(label, p, help_text=None):
    """
    Color-coded probability display: green/yellow/red based on how
    favorable the number is, plus a progress bar for a quick visual read.
    Thresholds: >=70% green, 40-70% yellow, <40% red.
    """
    if p >= 0.70:
        color = "🟢"
    elif p >= 0.40:
        ...
        color = "🔴"
    st.markdown(f"{color} **{label}:** {p:.1%}")
    if help_text:
        st.caption(help_text)
    st.progress(min(max(p, 0.0), 1.0))


def format_bid_visual(bid, bid_owner=None):
    """Bid display with a die-face icon for the face value."""
    if bid is None:
        return "(no bid yet)"
    face_icon = DIE_FACES.get(bid.face, str(bid.face))
    type_tag = " (ones)" if bid.bid_type == BidType.ONES else ""
    owner_str = f" — Player {bid_owner+1}" if bid_owner is not None else ""
    return f"**{bid.quantity}** × {face_icon}{type_tag}{owner_str}"
...
# ==================================================================
    ...
    if st.session_state.own_dice:
        st.markdown(f"### {dice_icons(st.session_state.own_dice)}")

    bid_owner_str = st.session_state.current_bid_owner
    st.markdown(f"**Current bid:** {format_bid_visual(st.session_state.current_bid, bid_owner_str)}")

    st.write("**Dice remaining:**")
    dc_cols = st.columns(min(num_players, 4))
    for i, c in enumerate(dice_counts):
        with dc_cols[i % 4]:
            you_tag = " (you)" if i == 0 else ""
            st.caption(f"P{i+1}{you_tag}")
            st.write(dice_count_icons(c))
    ...
    with tabs[0]:
        ...
        if raw.strip():
            try:
                ...
                if all(1 <= v <= 6 for v in preview_values):
                    st.markdown(f"Preview: ### {dice_icons(preview_values)}")
            except ValueError:
                pass
        ...
        if not st.session_state.own_dice:
            ...
            if st.button("Get recommended move", type="primary"):
                ...
                try:
                    ...
                    st.success(f"**Recommended: {action.action_type.value.replace('_', ' ').title()}**")
                    if action.resulting_bid:
                        st.markdown(f"Bid: {format_bid_visual(action.resulting_bid)}")
                    probability_badge("P(true)", action.probability,
                                       help_text="How likely this bid/challenge is to hold up if checked.")
    ...
    with tabs[2]:
        ...
        if not st.session_state.own_dice:
            ...
            if st.button("Check odds"):
                ...
                st.markdown(f"Checking: {format_bid_visual(Bid(qty, face, bt))}")
                probability_badge("P(true / safe)", report.p_true)
                probability_badge("P(false / vulnerable to Liar)", report.p_false)
                probability_badge("P(exactly this / Exact-callable)", report.p_exact)
    ...
    with tabs[3]:
        ...
        st.caption(f"Preview: {format_bid_visual(Bid(qty, face, preview_bt))}")
