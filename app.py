...
def pname(idx):
    """Resolve a player index to their display name, falling back to
    'Player N' if names haven't been set yet."""
    if idx is None:
        return None
    ...
    if idx < len(names) and names[idx]:
        ...
        base = f"Player {idx+1}"
    return base + (" (you)" if idx == 0 else "")


def player_option_labels(num_players):
    """
    Build a list of display labels for player-selection dropdowns,
    guaranteed unique even if two players share the same name (appends
    a disambiguating number in that case). Using plain string options
    here (rather than an index + format_func) is deliberate - it's a
    simpler, more robust pattern than pairing a raw value with a
    separate display formatter.
    """
    raw = [pname(i) for i in range(num_players)]
    seen = {}
    labels = []
    for label in raw:
...
def bid_display(bid, owner=None):
    ...
    if bid is None:
        st.markdown('<span style="color:#9ca3af; font-weight:600;">No bid yet this round</span>',
                    unsafe_allow_html=True)
        return
    owner_html = f'<span class="bid-owner">{pname(owner)}</span>' if owner is not None else ""
...
def bid_log_list():
    """Render the running list of bids made so far this round."""
    log = st.session_state.get("bid_log", [])
    if not log:
        st.caption("No bids recorded yet this round.")
        return
    for n, (owner_idx, bid) in enumerate(log, start=1):
        tile = _die_tile_html(bid.face, small=True)
        type_tag = "ones" if bid.bid_type == BidType.ONES else ""
        st.markdown(f"""
        <div style="display:flex; align-items:center; gap:10px; padding:6px 0;
                    border-bottom:1px solid {BORDER};">
          <span style="color:{TEXT_MUTED}; font-size:12px; width:18px;">{n}.</span>
          {tile}
          <span style="font-weight:700;">{bid.quantity} × {bid.face}</span>
          <span style="color:{TEXT_MUTED}; font-size:12px;">{type_tag}</span>
          <span style="margin-left:auto; color:{ACCENT}; font-weight:600; font-size:13px;">{pname(owner_idx)}</span>
        </div>
        """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("Game rules"):
        st.markdown("""
        **Setup**
        Each player starts with up to 5 dice (max 6 ever). Everyone
        rolls secretly at the start of every round - you only see
        your own dice.

        **Bidding**
        A bid is a claim like *"four 5s"* - meaning you believe at
        least that many dice showing that face exist across the
        **whole table**, not just your own hand. On your turn you
        raise the current bid, or challenge it.

        **Ways to raise**
        - Increase the quantity on the same face (e.g. four 5s → five 5s)
        - Keep the quantity but move to a higher face (e.g. four 5s → four 6s)
        - **Switch to ones:** halve the quantity (round up) and move to
          betting on 1s. Example: eight 5s → four 1s
        - **Switch from ones:** double the quantity and add one, moving
          off 1s to any face you choose. Example: four 1s → nine of any face

        **Wild 1s**
        Normally, 1s count toward ANY face bid, in addition to
        counting as themselves. Example: with wilds on, a hand of
        [1, 1, 3] has three dice that count toward a "3s" bid.

        **One Rule**
        The moment any player is down to their last die, 1s stop
        being wild for that entire round - they only count as
        literal 1s. Use the toggle on the main page to control this
        directly, since it can matter in edge cases the app can't
        always detect on its own.

        **Challenges**
        - **Call Liar:** you think the bid is too high. Reveal all
          dice - if the bid was false, the bidder loses a die; if it
          was actually true, you lose a die instead
        - **Call Exact:** you think the count is precisely right, not
          just "at least." Guess correctly and you *gain* a die
          (capped at 6); guess wrong and you lose one
...
if not st.session_state.setup_done:
    with st.container(border=True):
        ...
        num_players = st.number_input(
            ...
            help="Count everyone at the table, including yourself. You'll always be the first player listed."
        )

        st.write("Name and starting dice count for each player (usually 5 each):")
        dice_counts = []
        player_names = []
        cols = st.columns(min(num_players, 4))
        for i in range(num_players):
            with cols[i % 4]:
                default_name = "You" if i == 0 else f"Player {i+1}"
                name = st.text_input(
                    "Name", value=default_name, key=f"setup_name_{i}",
                    help="Shown throughout the app instead of a plain player number."
                )
                d = st.number_input(
                    "Dice", min_value=0, max_value=6, value=5, key=f"setup_dice_{i}",
        ...
        if st.button("Start game", type="primary"):
            ...
            players = [Player(id=i, name=player_names[i]) for i in range(num_players)]
...
# ==================================================================
    ...
    auto_one_rule = any(c == 1 for c in dice_counts if c > 0)

    # One Rule: manual override, defaulting to the auto-detected value.
    # Auto-detection only reflects a single snapshot in time and can't
    # account for a player remaining on 1 die across multiple rounds in a
    # row (or any other house-rule nuance) - so this is fully user-controlled,
    # with the auto value shown alongside as a hint rather than as the source
    # of truth.
    if "one_rule_manual" not in st.session_state:
        st.session_state.one_rule_manual = auto_one_rule
    ones_wild = not st.session_state.one_rule_manual

    # --- Status card ---
    with st.container(border=True):
        c1, c2, c3 = st.columns(3)
        c1.metric("Total dice in play", total_dice)
        c2.metric("Your dice", "set" if st.session_state.own_dice else "not set")
        c3.metric("Ones wild?", "No" if st.session_state.one_rule_manual else "Yes")

        st.toggle(
            "One Rule active this round (1s NOT wild)",
            value=st.session_state.one_rule_manual,
            key="one_rule_manual",
            help=f"You control this directly. Auto-detected from dice counts: "
                 f"{'active' if auto_one_rule else 'not active'} (someone is on "
                 f"exactly 1 die: {'yes' if auto_one_rule else 'no'}). Override if "
                 f"it doesn't match what's actually happening at your table."
        )
        if st.session_state.one_rule_manual != auto_one_rule:
            st.caption("⚠️ Manually overridden - doesn't match the auto-detected value.")
    ...
    # --- Dice remaining card (also handles manual count edits) ---
    with st.container(border=True):
        st.markdown('<div class="card-label">Dice remaining</div>', unsafe_allow_html=True)
        st.caption("Adjust a count directly if it ever gets out of sync with the table.")
        dc_cols = st.columns(min(num_players, 4))
        for i, c in enumerate(dice_counts):
            with dc_cols[i % 4]:
                st.markdown(f'<div class="player-chip-label">{pname(i)}</div>',
                            unsafe_allow_html=True)
                ghost_dice_row(c)
                new_c = st.number_input(
                    "Dice", min_value=0, max_value=6, value=c, key=f"dice_count_{i}",
    ...
    tabs = st.tabs(["1️⃣ Your dice", "2️⃣ Recommendation", "🔍 Check odds",
                     "📝 Record bid", "⚖️ Resolve challenge"])
    ...
    with tabs[3]:
        ...
        player_labels = player_option_labels(num_players)
        selected_label = st.selectbox(
            "Who made this bid?", options=player_labels,
            index=min(1, num_players - 1), key="rec_owner",
        )
        owner = player_labels.index(selected_label)
        preview_bt = BidType.ONES if face == 1 else BidType.REGULAR
        st.caption("Preview:")
        bid_display(Bid(qty, face, preview_bt))
        if st.button("Record bid"):
            bt = BidType.ONES if face == 1 else BidType.REGULAR
            new_bid = Bid(quantity=qty, face=face, bid_type=bt)
            st.session_state.current_bid = new_bid
            st.session_state.current_bid_owner = owner
            st.session_state.bid_log.append((owner, new_bid))
            st.success(f"Current bid updated to {qty}x face {face} ({pname(owner)})")
            st.rerun()

        st.divider()
        st.markdown("**Bids so far this round:**")
        bid_log_list()
    ...
    with tabs[4]:
        ...
        if st.session_state.current_bid is None:
            ...
            caller_labels = player_option_labels(num_players)
            selected_caller_label = st.selectbox(
                "Who made the call?", options=caller_labels, key="challenge_caller",
            )
            caller = caller_labels.index(selected_caller_label)
            ...
            if st.button("Resolve challenge", type="primary"):
                ...
                if outcome == "Liar":
                    ...
                    if was_true:
                        ...
                        msg = f"Bid was TRUE. {pname(caller_idx)} loses a die."
                    elif owner_id is not None:
                        dice_counts[owner_id] = max(0, dice_counts[owner_id] - 1)
                        msg = f"Bid was FALSE. {pname(owner_id)} loses a die."
                else:
                    ...
                    if was_exact:
                        ...
                        msg = f"EXACT! {pname(caller_idx)} gains a die."
                    else:
                        dice_counts[caller_idx] = max(0, dice_counts[caller_idx] - 1)
                        msg = f"Not exact. {pname(caller_idx)} loses a die."
                ...
                if len(remaining) <= 1:
                    ...
                    st.balloons()
                    msg += f"  🏆 Game over! Winner: {pname(winner)}" if winner is not None else "  Game over."
